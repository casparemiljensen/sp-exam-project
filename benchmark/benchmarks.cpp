...
static void circadian_rhythm_single_core_10_runs(benchmark::State &state) {
    auto vessel = Examples::circadian_rhythm();

    debug_print("\nStarting circadian_rhythm_single_core_10_runs");
    for (const auto _: state) {
        Examples::get_peak_average_serial(1500, vessel, 10, "DA");
        // This finds peak average of H - H does not exist in all examples
    }
    debug_print("End circadian_rhythm_single_core_10_runs");
}

BENCHMARK(circadian_rhythm_single_core_10_runs)->Unit(benchmark::kMillisecond);

static void circadian_rhythm_single_core_10_runs_optimized(benchmark::State &state) {
    auto vessel = Examples::circadian_rhythm();

    debug_print("\nStarting circadian_rhythm_single_core_10_runs_optimized");
    for (const auto _: state) {
        Examples::get_peak_average_serial_optimized(1500, vessel, 10, "DA");
        // This finds peak average of H - H does not exist in all examples
    }
    debug_print("End circadian_rhythm_single_core_10_runs_optimized");
}

BENCHMARK(circadian_rhythm_single_core_10_runs_optimized)->Unit(benchmark::kMillisecond);
...
BENCHMARK(circadian_rhythm_multi_core_100_runs)->Unit(benchmark::kMillisecond);
